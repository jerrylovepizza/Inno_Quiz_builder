...
export class Controller extends Component {
    ...
    static getDerivedStateFromProps(props, state) {
        let font = []
        props.settings.font.map((val, index) => {
    ...
    render() {
        ...
        const fontstyle = this.props.settings.font.map((val, index) => {return create_font_style(val, index); });
        const colorstyle = this.props.settings.color.map((val, index) => { return create_color_style(val, index)});
        const parastyle = this.props.settings.paragraph.map((val, index) => { return create_para_style(val, index)});
