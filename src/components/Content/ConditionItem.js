...
const con_operators = [
    "is blank","is not blank", "contains" , "does not contains", "equals", "does not equal"
...
const selectedTarget = (values, e) => {

    const sel = values.filter((val) => {
        return e == val.value;
    });
...
return (
    <components.DropdownIndicator {...props}>
        ...
        <path d="M7.0394 7L14.0788 0H0L7.0394 7Z" fill="#DADADA"/>
        </svg>
...
class ConditionItem extends Component {
  constructor(props) {
    ...
    let questions = this.props.questions.map((val) => {
        ...
        return {
            ...
            label: (<div class="qr-option">{question_item(val.type)} <ContentEditable            
              html={val.title} 
              disabled={true} tagName="span" /></div>)
        }
    });
    ...
    this.state = {
        ...
        selected : this.props.result.target,
        scondition: this.props.result.condition,
    }
  };

  onChangeTarget(e) {
    if (e.value == this.state.selected) {
        return;
    }
    ...
    this.props.result.target = e.value;
  ...
  onChangeCondition(e) {
    ...
    this.props.result.condition = e.value;
    this.props.changeResult({
        type: "CHANGE",
        index: this.props.point,
        result: this.props.result
  ...
  onChange(e) {
    this.props.result.redirect = e.target.value;
    this.props.changeResult({
        type: "CHANGE",
        index: this.props.point,
        result: this.props.result
  ...
  onChangex(e) {
    this.props.result.val[0] = e.target.value;
    this.props.changeResult({
        type: "CHANGE",
        index: this.props.point,
        result: this.props.result
  ...
  onChangey(e) {
    this.props.result.val[1] = e.target.value;
    this.props.changeResult({
        type: "CHANGE",
        index: this.props.point,
        result: this.props.result
  ...
  onChangeOp(e) {
    this.props.result.sum = e.target.value;
    this.props.changeResult({
        type: "CHANGE",
        index: this.props.point,
        result: this.props.result
  ...
  render() {
    const [x, y] = this.props.result.val;
    const redirect = this.props.result.redirect;
    const sum = this.props.result.sum;
    let operator = isNaN(this.state.selected) ? (
        <Fragment><span>is</span>
                <div class="r_select r150">
                <Select ref='seltype' options={conditions} 
                        styles={colourStyles}
                        onChange={(e) => {this.onChangeCondition(e)}}
                        components={{DropdownIndicator, IndicatorSeparator: () => null}}
                        value={selectedTarget(conditions, this.state.scondition)}
                        isSearchable={false}
                        />
                ...
                <div class="r_select r150">
                <Select ref='seltype' options={con_operators} 
                        styles={colourStyles}
                        onChange={(e) => {this.onChangeCondition(e)}}
                        components={{DropdownIndicator, IndicatorSeparator: () => null}} 
                        value={selectedTarget(con_operators, this.state.scondition)}
                        isSearchable={false}
                        />
    ...
    return (
        <Fragment>
            { (this.props.point > 0) ? (
                <div class="r_operator">
                <select class="r_op qr_select" onChangeOp = {(e) => {this.onChangeOp(e); }}>
                    <option value="and" selected={ sum == "and" ? "selected" : "" }>And</option>
                    <option value="or" selected={ sum == "or" ? "selected" : "" }>Or</option>
                </select>
            ...
            <div class="r_item">
                <span>If</span>
                <div class="r_select r200">

                        <Select ref='seltype' options={this.state.target} 
                        styles={colourStyles}
                        onChange={(e) => {this.onChangeTarget(e)}}
                        components={{DropdownIndicator, IndicatorSeparator: () => null}}                        
                        value={selectedTarget(this.state.target, this.state.selected)}
                        isSearchable={false}
                        />
                ...
                {
                    (this.state.scondition == "between") ? 
                    (
                        <Fragment>
                        <div class="r_val">
                            <input type="text" name="r_val" class="q-control" value={x}  onChange={(e) => {this.onChangex(e);}}></input>
                        </div>
                        <span>and</span>
                        <div class="r_val">
                            <input type="text" name="r_val1" class="q-control" value={y}  onChange={(e) => {this.onChangey(e);}}></input>
                        </div>
                        </Fragment>
                    )
                    :
