...
return (
  ...
  <path d="M7.0394 7L14.0788 0H0L7.0394 7Z" fill="#32373C"/>
  </svg>
...
class FieldItem extends Component {
  ...
  onChange(e, type) {
      ...
      if (type == "m") {
        this.props.mailer.m_id = val;
      } else {
        this.props.mailer.q_id = val;
      }

    this.props.changeMailer({
        index: this.props.pos,
        mail: this.props.mailer,
    });
  }
  render() {
        const m_id  = this.props.mailer.m_id;
        const q_id = this.props.mailer.q_id;
        
    return (

        <div class="mf_item">

            <Select className="mf_sel" ref='selFields' options={selFields(this.props.fields)} 
                        styles={colourStyles}
                        onChange={(e) => {this.onChange(e, "m")}}
                        components={{DropdownIndicator, IndicatorSeparator: () => null}}
                        value={selectedTarget(selFields(this.props.fields), m_id)}
                        isSearchable={false}
                        />

            <span class="mf-connector"> </span>

            <Select className="mf_sel" ref='selOptions' options={selOptions(this.props.questions)} 
                        styles={colourStyles}
                        onChange={(e) => {this.onChange(e, "q")}}
                        components={{DropdownIndicator, IndicatorSeparator: () => null}}
                        value={selectedTarget(selOptions(this.props.questions), q_id)}
                        isSearchable={false}
                        />
        </div>
    );
